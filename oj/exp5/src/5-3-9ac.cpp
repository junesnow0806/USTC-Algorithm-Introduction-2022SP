...
struct BigInteger {
    ...
    pair<BigInteger, BigInteger> operator/(const BigInteger &bi) {
        ...
        // 使用减法计算除法
        r = self;
        for (int i = self.len - bi.len; i >= 0; i--) {
            // 计算商的第i位
...
BigInteger extend_gcd(BigInteger a, int b, int &x, int &y) {
    ...
    BigInteger bi(to_string(b));
    // BigInteger gcd = extend_gcd(b, a%b, y, x);
    BigInteger tmp = a % bi;
    BigInteger gcd = extend_gcd(bi, Big2int(tmp), y, x);
    int tmp2 = Big2int(a / b * x);
    y = y - tmp2;
    return gcd;
}
